...
public class BlockModDoor extends BlockMod {
    ...
    public IIcon getIcon(IBlockAccess blockAccess, int x, int y, int z, int side) {
        if (side != 1 && side != 0) {
            /* TODO: All of these variables should be named with a name that can actually be understood */
            int i1 = this.reallyBrock_ThisIsJustGettingSillyNow_StopFingCopyingCode(blockAccess, x, y, z); //TODO: WTF does this method do?
            int j1 = i1 & 3;
            boolean flag = (i1 & 4) != 0;
            boolean flag1 = false;
    ...
    public boolean getBlocksMovement(IBlockAccess blockAccess, int x, int y, int z) {
        int l = this.reallyBrock_ThisIsJustGettingSillyNow_StopFingCopyingCode(blockAccess, x, y, z);
        return (l & 4) != 0;
    }
    ...
    public int nameThisBetterBitwise(IBlockAccess world, int x, int y, int z) {
        return this.reallyBrock_ThisIsJustGettingSillyNow_StopFingCopyingCode(world, x, y, z) & 3;
    }

    public boolean isNameThisBetter(IBlockAccess blockAccess, int x, int y, int z) {
        return (this.reallyBrock_ThisIsJustGettingSillyNow_StopFingCopyingCode(blockAccess, x, y, z) & 4) != 0;
    }
        ...
        if ((l & 8) == 0) {
            ...
            if (flag) {
                ...
                if ((flag1 || block.canProvidePower()) && block != this)
                    this.brockStopCopyingCodeAndNameThis_youWroteItYouShouldKnowWhatItDoes(world, x, y, z, flag1);
            }
        } else {
            if (world.getBlock(x, y - 1, z) != this)
                world.setBlockToAir(x, y, z);
    ...
    public int reallyBrock_ThisIsJustGettingSillyNow_StopFingCopyingCode(IBlockAccess blockAccess, int x, int y, int z) {
